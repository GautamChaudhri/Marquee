...
import { getRankingResiduals, getTasteMap, getTasteProfiles, getTasteStatus } from '$lib/api/taste';
...
export const load: PageLoad = async ({ fetch, url }) => {
	...
	try {
		const [status, mapData, profiles, residuals] = await Promise.all([
		...
		return { status, mapData, profiles, residuals, library, error: null as string | null };
	} catch (e) {
		return {
