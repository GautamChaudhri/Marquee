...
5. `auth.py:119` has a TODO: *"Frontend phase will accept a valid httpOnly session cookie here."* — cookie auth is **not built**.
...
*Companion to `MARQUEE_HANDOFF.md` (design system, components) and `Marquee.dc.html` (visual reference only — proprietary runtime + synthetic data, not portable).*
