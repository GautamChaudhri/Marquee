"""JMC3B confined physical and deterministic virtual artifact contracts."""
...
async def _attempt(db, label: str):
    job_id = uuid.uuid5(uuid.NAMESPACE_URL, f"jmc3b-artifact:{label}").hex
    job = Job(
        id=job_id,
        type="system_noop",
        payload_version=1,
...
def _source(key: str, content: bytes):
    ...
    directory = boundary.from_key("data", "jmc3-artifact-sources")
    boundary.create_directory(directory, parents=True)
    source = boundary.from_key("data", f"jmc3-artifact-sources/{key}")
    ...
    return boundary.classify(
        settings.data_dir_path / "jmc3-artifact-sources" / key,
        roots=("data",),
        require_file=True,
    )
...
async def test_physical_artifact_is_confined_immutable_and_downloadable(db) -> None:
    ...
    assert row.storage_key and row.storage_key.startswith("jmc3/evidence/artifacts/")
