        ...
        PosterPipelineGroupRequestV1.model_validate(
            {
                "library": "movies",
                "chunk_index": 0,
                "members": [_movie(index) for index in range(1, 18)],
            }
        )
...
def test_group_definition_is_enabled_gpu_leaf_with_partial_success_alias() -> None:
    ...
    assert {"POSTER_GROUP_ENABLED", "POSTER_GROUP_CHUNK_SIZE"} <= definition.configuration_keys
...
def test_group_rollout_configuration_is_database_owned_and_hard_bounded() -> None:
    settings = PipelineSettings(POSTER_GROUP_ENABLED=True, POSTER_GROUP_CHUNK_SIZE=16)
    ...
    for key in ("POSTER_GROUP_ENABLED", "POSTER_GROUP_CHUNK_SIZE"):
    ...
    assert {"POSTER_GROUP_ENABLED", "POSTER_GROUP_CHUNK_SIZE"} <= set(downloads["knobs"])
