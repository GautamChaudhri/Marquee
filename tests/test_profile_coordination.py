"""Taste-profile build coordination across libraries and generations.

Exactly one profile build may be in flight per library, enforced by a partial unique index.
Covers explicit generations, terminal failure until a canonical retry, cancellation
lineage, newer evidence coalescing into one successor, active profiles staying personalized
when a later update fails, and readiness requiring a live consumer acknowledgement."""
...
def _job(*, job_type: str = "poster_deploy") -> Job:
    ...
    return Job(
        ...
        subject_kind="movie",
        subject_reference="profile",
        subject_snapshot={"version": 1, "kind": "movie", "title": "Profile fixture"},
    )
...
async def _seed_required_positive_subjects(db, *, count: int = 50) -> None:
    ...
    for index in range(count):
        digest = sha256(f"profile-{index}".encode()).hexdigest()
        artifact = JobArtifact(
            job_id=job.id,
            kind="taste_exemplar",
            name=f"example-{index}.jpg",
            status="available",
            storage_key=f"profiles/profile/{index}.jpg",
            content_type="image/jpeg",
            size_bytes=1,
            checksum=digest,
            retention_class="pinned",
            artifact_metadata={"fixture": "profile"},
        )
        db.add(artifact)
    ...
    for index, digest, artifact in artifacts:
        event = PosterPreferenceEvent(
            id=uuid4().hex,
            version=1,
            idempotency_key=f"profile:positive:{index}",
            namespace="global",
            subject_kind="movie",
            ...
            training_context={"fixture": "profile"},
            confidence="explicit",
            initiator={"kind": "test"},
        )
        db.add(event)
...
async def _activate_build(db, build: TasteProfileBuild, *, generation: int) -> None:
    ...
    storage_key = f"profiles/{job.id}-{build.library}-{generation}.npz"
...
async def test_initial_profile_builds_are_one_per_library_with_explicit_generation(
    ...
    assert await schedule_initial_profile_build(db, initiator_identifier="profile-build-test") == ()
...
async def test_readiness_requires_a_live_current_consumer_acknowledgement(
    ...
    replacement = RuntimeInstance(
        id=str(uuid4()),
        role="worker",
        node_label="profile-replacement",
        build="test",
        host_boot_id=str(uuid4()),
        process_id=2001,
        process_start_ticks=1,
