"""JMC3B bounded, redacted, attempt-owned log evidence contracts."""
...
def test_contract_redaction_matrix() -> None:
    fixture = json.loads(
        (Path(__file__).parent / "fixtures/jmc3b/b0_contract_freeze.json").read_text()
    )["redaction"]
...
async def test_sink_captures_all_sources_redacts_cross_chunk_and_seals(db) -> None:
    ...
    test_logger = logging.getLogger("marquee.tests.jmc3b.capture")
...
async def test_python_capture_is_scoped_and_does_not_cross_contaminate(db) -> None:
    ...
    scoped_logger = logging.getLogger("marquee.tests.jmc3b.scoped")
...
async def test_log_list_stream_and_download_contract(db) -> None:
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        download = await client.get(
            f"/api/jobs/{job.id}/attempts/{attempt.id}/logs/download"
        )
        ...
        assert (
            await client.get(f"/api/jobs/other/attempts/{attempt.id}/logs")
        ).status_code == 404
