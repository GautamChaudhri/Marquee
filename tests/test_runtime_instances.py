"""Runtime instances: durable incarnation lifecycle and activation safety.

A runtime instance is topology evidence, never queue authority. Covers worker and scheduler
incarnation heartbeat and stop, bounded heartbeat failure, sanitized capability
advertisement, the exact incarnation recorded on an admitted attempt, and the
release-blocking guard that deferred routes stay unmounted rather than silently
auth-exempt."""
...
async def test_worker_and_scheduler_incarnations_heartbeat_and_stop(db) -> None:
    ...
    await db.rollback()
    rows = list(await db.scalars(select(RuntimeInstance).order_by(RuntimeInstance.role.desc())))
