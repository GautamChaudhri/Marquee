...
async def test_fixed_canary_integrates_progress_logs_artifacts_events_and_apis(db) -> None:
    ...
    attempt = (await db.scalars(select(JobAttempt).where(JobAttempt.job_id == job_id))).one()
    log = (await db.scalars(select(JobLog).where(JobLog.job_id == job_id))).one()
    artifact = (await db.scalars(select(JobArtifact).where(JobArtifact.job_id == job_id))).one()
    events = list(
    ...
    assert {"attempt.started", "job.succeeded", "log.available", "artifact.available"} <= set(keys)
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        ...
        log_download = await client.get(f"/api/jobs/{job_id}/attempts/{attempt.id}/logs/download")
        artifact_page = await client.get(f"/api/jobs/{job_id}/artifacts")
        artifact_download = await client.get(f"/api/jobs/{job_id}/artifacts/{artifact.id}/download")
    ...
    assert artifact_download.json()["value"]["summary"]["echo"] == "integrated"
