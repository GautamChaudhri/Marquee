...
async def test_metrics_purge_is_bounded_confirmed_and_cancellable(
    ...
    recent = SystemMetricsSample(cpu={}, ram={}, disk={}, net={}, active_jobs=[], created_at=now)
    db.add_all([old, recent])
    await db.commit()
    await db.refresh(old)
    await db.refresh(recent)

    context = _context({"dry_run": True, "retention_days": 30, "max_records": 10, "batch_size": 1})
        ...
        remaining = set(
            (
                await session.scalars(
                    select(Job.id).where(Job.id.in_([eligible.id, protected.id, child.id]))
                )
            ).all()
        )
