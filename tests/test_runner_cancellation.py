"""Runner-host cancellation: every exit path after launch is bounded.

A contained runner must never be abandoned alive. An outer Task.cancel() from worker
shutdown, the delivery timeout, or an exception raised inside the progress callback all
enter cleanup with the runner's own cancelled/timed-out flags unset, so each path has to
perform bounded cooperative/TERM/KILL cleanup, confirm owned-tree death, and preserve the
original cancellation or exception classification."""
...
async def test_outer_delivery_timeout_terminates_runner_tree(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert isinstance(task.exception(), TimeoutError), (
            f"the delivery timeout classification must be preserved, got {task.exception()!r}"
        )
