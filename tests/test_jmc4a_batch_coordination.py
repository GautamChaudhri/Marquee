"""A3 dynamic sealing, ticketless aggregation, repair, and parent commands."""
...
def _registry(*, fixed: bool, retry_children: str = "failed") -> JobDefinitionRegistry:
    ...
    parent = replace(
        ...
        parent_policy=ParentAggregationPolicy(
            fixed_children=fixed, retry_children=retry_children
        ),
...
def test_locked_parent_outcome_matrix(outcomes, parent_cancelled, expected) -> None:
    ...
    assert (
        batch_module._aggregate_outcome(counts, parent_cancelled=parent_cancelled)
        == expected
    )
...
@pytest.mark.asyncio
async def test_dynamic_open_append_retry_and_permanent_seal(
    db, dynamic_registry
) -> None:
    async with db.begin():
...
@pytest.mark.asyncio
async def test_dynamic_empty_seal_is_no_change_without_transport(
    db, dynamic_registry
) -> None:
    async with db.begin():
...
async def test_open_batch_cannot_terminalize_and_seal_applies_locked_outcome(
    ...
    async with db.begin():
        ...
        assert (parent.phase, parent.outcome) == ("running", None)
        sealed = await seal_dynamic_batch(
            db, parent_id=parent.id, generation=opened.generation
        )
...
@pytest.mark.asyncio
async def test_projection_repair_is_compare_and_set_and_event_sparse(
    db, dynamic_registry
) -> None:
    async with db.begin():
...
@pytest.mark.asyncio
async def test_concurrent_terminal_projection_counts_each_child_once(
    db, monkeypatch
) -> None:
...
@pytest.mark.asyncio
async def test_concurrent_dynamic_append_and_seal_are_serialized(
    db, dynamic_registry
) -> None:
    async with db.begin():
    ...
    assert await db.scalar(
        select(func.count(Job.id)).where(Job.parent_id == opened.parent.job_id)
...
async def test_parent_priority_cancel_and_retry_target_only_direct_children(
    ...
    child_rows = (
        await db.scalars(select(Job).where(Job.parent_id == created.parent.job_id))
    ).all()
    ...
    cancelled = await cancel(
        db, job_id=created.parent.job_id, expected_fence_token=1
    )
    ...
    await db.rollback()
    successor = await retry(
        db, job_id=created.parent.job_id, expected_fence_token=2
    )
