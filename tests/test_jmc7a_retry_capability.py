"""JMC7A retry capability truth is definition-owned, not enabled-state guessed."""
...
def test_generic_retry_requires_the_canonical_subject_and_bounded_request() -> None:
    ...
    assert resolve_retry_capability(missing_subject, definition).reason == "bounded_retry_subject_unavailable"

    invalid_request = _terminal_job("system_noop", request={"unbounded": object()})
    assert resolve_retry_capability(invalid_request, definition).reason == "bounded_retry_request_unavailable"
