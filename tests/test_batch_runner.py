...
from marquee.pipeline.batch_runner import (
    AssetSpec,
    _BatchMovie,
    _detail_batch,
    _download_phase,
...
def test_detail_batch_score_failure_is_isolated_to_one_candidate(
    ...
    monkeypatch.setattr(
        batch_runner, "select_scorer", lambda *args, **kwargs: _StubDiagnosticScorer()
    )
...
# ---------------------------------------------------------------------------
# apply_official_pick (design 04 §9.3)
# ---------------------------------------------------------------------------


def _stacked_candidate(name: str, *, stack_id: int, stack_rank: int, stack_size: int) -> CandidateScore:
...
# ---------------------------------------------------------------------------
# AssetSpec / run_batch_assets (design 04 §9.2)
# ---------------------------------------------------------------------------


def test_asset_spec_defaults_preserve_movie_shape():
    asset = AssetSpec(media_type="movie", subject_id=7, title="Die Hard", tmdb_id=562)
    assert asset.series_id is None
    assert asset.season_id is None
    assert asset.season_number is None
    assert asset.ocr_title is None
