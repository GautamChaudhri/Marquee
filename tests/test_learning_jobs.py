"""Learning jobs must leave a real product effect, not a placeholder.

Each of these asserts that a handler actually changed the product: the poster pipeline
writes a canonical PipelineRun projection, taste rebuild and enrichment publish a native
artifact the real loader can consume and advance the consumed profile authority, the taste
map consumes the active profile, and residual training either publishes a loadable artifact
or records a verified no-change without failing delivery."""
...
async def test_poster_pipeline_writes_canonical_pipeline_run_projection(
    ...
    certified separately by ``test_live_smoke.py``) so the handler's
...
async def test_taste_rebuild_publishes_native_loadable_profile_artifact(db, data_dir, monkeypatch):
    ...
    ``test_live_smoke.py``). The handler validates it with the production
...
async def test_ranking_residual_publishes_native_loadable_artifact(db, data_dir, monkeypatch):
    ...
    monkeypatch.setattr("marquee.core.jobs.handlers_ml.resolve_active_publication", fake_resolve)
